# Space Sim — explorations/space-sim

A modular 2D orbital physics simulation rendered in SVG. Used in the Explorations essay page
(`index.html`) as a series of embedded demonstrations, and in a companion interactive sandbox
(`sandbox.html`) for tuning the "Hail Mary" setup.
...
8 px/s. Use `false` for setups that need a fixed orientation (hover, falling, hail-mary).
