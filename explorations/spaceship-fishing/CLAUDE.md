...
| `fishing-adrian-orbit` | Big planet. Rocket spawns above the viewport on a tangent line to the orbit (offset = `ADRIAN_ORBIT_R` left of planet centre), nose already up, falls tail-first with `vx` forced to 0 each step so the path stays a clean vertical line. The controller starts the insertion burn when the remaining vertical distance to the 9 o'clock point equals the 1-D braking distance `(v.y² − V²) / (2·T)` — this makes the brake *finish* with velocity = orbital velocity right at `dy = 0` instead of starting at that altitude and overshooting. |
