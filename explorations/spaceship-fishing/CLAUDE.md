...
| `fishing-adrian-orbit` | Big planet. Pure-physics insertion — no velocity overrides anywhere. Rocket spawns at the apoapsis of a bound elliptical trajectory whose periapsis is at `ADRIAN_ORBIT_R`. Apoapsis is set to `R_APO = 600 px` (off-screen right on typical viewports); `v_apo` is back-solved from conservation of E and L as `√(2·g·r_peri² / (r_apo + r_peri))`. Gravity does all the bending. The controller's state machine has three phases: **approach** (coast, watch for periapsis), **burn** (apply ΔV = `v_circ·t̂ − v` until small), **orbit** (coast). Orientation timeline during approach: hold nose-up (prograde at spawn) while the rocket is still off-canvas, latch the flip start the first frame `rocket.position.x < bounds.width`, `linearRotate` 180° clockwise over 2 s, then track anti-prograde for the rest of the approach. The visibility-triggered flip works on both narrow viewports (where the rocket drifts in late) and wide viewports (where it spawns already on screen) — fixed-time gating would leave the rocket nose-up for several seconds on wide panels so the engine is already pointed in the direction of motion when periapsis arrives — this keeps the burn-start snap small (the ΔV at periapsis is anti-tangent ≈ anti-prograde) and the resulting orbit is held ass-first to match `fishing-adrian-slowdown`. Periapsis is detected when radial velocity flips from negative to positive, with a `dist < 0.95·R_APO` guard to suppress spurious triggers at the apoapsis start. |
