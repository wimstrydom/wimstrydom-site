...
- **Scroll-driven chart (`.scrolly`):** the "Why Now?" section is a scrollytelling figure. The `.scrolly-fig` is **full-width and pinned to the top** (`position: sticky; top: var(--header-h)`, bled to the content edges with `margin: 0 -48px`); the `.scrolly-step` prose blocks (ordinary prose spacing) scroll beneath it. It holds **two inline SVGs** — `.fig-desktop` (landscape, `viewBox 640×400`) and `.fig-mobile` (portrait, `viewBox 600×520`, ~3× taller) — swapped by a `≤480px` media query so the chart is legible on phones; both carry identical `.fig-layer` `data-layer` groups so one JS drives both. Below the plot, an **HTML legend** (`.fig-legend`, wraps responsively) carries the descriptive series labels (Cost / Housemate Tracker Value / Bulgarian Customs Value / Calculator Value) — kept out of the SVG so long labels don't collide with the curves. Each step carries a `data-state` (0–6); the chart state is **derived deterministically from scroll position** (a trigger line just below the pinned chart, in the page `<script>`) rather than via IntersectionObserver, so it is always correct for where the reader is. Layers build up cost curve → value lines → availability → selection points → cost drops → availability widens. Each app's "selection" marker sits at the **right edge of the shaded available region** (the cheapest still-available point), with `.fig-note` annotations ("viable" / "not viable"). **The SVGs use CSS classes (not presentation attributes) for all colour, so they read the real design tokens — no hardcoded hex and no JS colour mirror needed** (unlike the spin-station canvas). Value lines map yellow→`--amber`, blue→`--blue`, green→`--green`; cost→`--red`. **Both SVGs share the same validated geometry (the mobile one is an affine remap); keep them in sync.** The default (no-JS) state shows the final layer set.
